...
model = build_model(
    ...
    num_transformer_decoderblocks=range(0,1),  #range(0,3)
    mlp_dropout=0.2,
    dropout=0.2,
)
...
#meanmae_error=np.mean(abs(test_predict- np.array(y_test)))
#test_rmse = math.sqrt(mean_squared_error(test_predict, np.array(y_test)))
