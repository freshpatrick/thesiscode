...
for i in range(0,len(stock_id)):  #len(stock_id)
    ...
    #若資料長度為45就繼續，否則就跳過  7/1後要改成45 這邊先改44
    if(len(temp_stockdata)==45): #原本if(len(temp_stockdata)==44):
        #重設temp_stockdata3 index
        index1 = pd.Index(list(range(45)))  # index1 = pd.Index(list(range(44))) 
        ...
        temp_stockdata3=pd.DataFrame(temp_stockdata.iloc[44,:]).transpose()
