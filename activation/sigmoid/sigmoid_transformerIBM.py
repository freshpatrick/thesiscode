...
def transformer_encoder(inputs, head_size, num_heads, ff_dim, dropout=0):
    ...
    x_1 = layers.Conv1D(filters=4,kernel_size=1,padding='causal', dilation_rate=2,activation='sigmoid')(x)
    x_2 = layers.Conv1D(filters=4,kernel_size=1,padding='causal', dilation_rate=4,activation='sigmoid')(x)
    #兩個分支結合
...
def build_model(
    ...
    outputs = layers.Dense(1, activation="sigmoid")(x) #this is a pass-through
...
model = build_model(
    ...
    num_heads=2,  #4
    ff_dim=4,
    num_transformer_blocks=1,  #IBM用2 validation會比較變動  4
    #mlp_units=[128],
    mlp_units=range(0,1), #mlp_units=range(0,5),
    mlp_dropout=0.4,
    dropout=0.25,
)
