...
import pandas as pd
import torch
X = pd.read_csv('Dataset/AAPL.csv', index_col="Date", parse_dates=True)
...
model_args = ModelArgs(args.get("d_in"), args.get("num_layers"), args.get("num_nodes"), args.get('lag'), args.get('horizon'))

# Initialize the SAMBA model
model = SAMBA(model_args, args.get('hid'), args.get('lag'), args.get('horizon'), args.get('embed_dim'), args.get("cheb_k"))

# Create a dummy input tensor to match the input dimensions the model expects

#試試看x_train的tensor
input_tensor = torch.randn(32, args.get('seq_in'))  # Adjust this based on your model's input size
