...
#caculate T  T+1  T+2  T+3 MAE
test_indexs_new=test_indexs+1
test_indexs_month=test_indexs_new %12

#合併位置和MAE
testmae=pd.concat([pd.DataFrame(test_indexs_month),pd.DataFrame(y_test_orign),pd.DataFrame(y_pred)], axis=1)
testmae.columns=['month','y_test','y_pred']
...
t_num=[1,4,7,11]
for i in range(0,4):
    differ_t=testmae[testmae['month']==t_num[i]]
    sum_differ=sum(abs(differ_t['y_pred']-differ_t['y_test']))
    sum_differ_t=sum_differ_t+sum_differ
    lendiffer_t=lendiffer_t+len(differ_t)

mae_t=sum_differ_t/lendiffer_t  
mae_tdays.append(mae_t)
...
t_num=[2,5,8,12]
for i in range(0,4):
    differ_t1=testmae[testmae['month']==t_num[i]]
    sum_differ1=sum(abs(differ_t1['y_pred']-differ_t1['y_test']))
    sum_differ_t1=sum_differ_t1+sum_differ1
    lendiffer_t1=lendiffer_t1+len(differ_t1)

mae_t1=sum_differ_t1/lendiffer_t1 
mae_tdays.append(mae_t1)
...
t_num=[3,6,9,1]
for i in range(0,4):
    differ_t2=testmae[testmae['month']==t_num[i]]
    sum_differ2=sum(abs(differ_t2['y_pred']-differ_t2['y_test']))
    sum_differ_t2=sum_differ_t2+sum_differ2
    lendiffer_t2=lendiffer_t2+len(differ_t2)

mae_t2=sum_differ_t2/lendiffer_t2 
mae_tdays.append(mae_t2)

#T+3
differ_t3=testmae[testmae['month']==2]
sum_differ3=sum(abs(differ_t3['y_pred']-differ_t3['y_test']))
mae_t3=sum_differ3/len(differ_t3) 
mae_tdays.append(mae_t3)

mae_tdays=pd.DataFrame(mae_tdays)
mae_tdays.index=["T","T+1","T+2","T+3"]
mae_tdays.to_csv(r'D:/pytorch範例/transformer_tensorflow/1002transformer/dayscode/Tdays/mae_tdays.csv')