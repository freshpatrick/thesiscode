...
##########0424從這開始改#############
for i in range(191,len(unique_stockid)):#len(unique_stockid) 先爬十家
    print(i)


    #now_stockid=str(stockid['股票代碼'][i])#現在股票代碼
    now_stockid=str(unique_stockid.iloc[i,0])#現在股票代碼 找414支股票
    ...
    s3=json.loads(res3.text)#將資料轉成json格式
    ...
    s5=json.loads(res5.text)#將資料轉成json格式
    ...
    s6=json.loads(res6.text)#將資料轉成json格式
...
###################開始將資料存起來######################
    ...
    #如果沒有45筆ROEDATA就跳過
    if(len(finance_data)!=45):   ##原本是44 但5月份有第1季所以4
      print("跳過")
      continue
...
#################4月份要補2024-3  #######載入這邊先封起來################# 
 ###讀取月營收########   
 ##########寫入資料######################
month=['2024-05']  #第44筆是第一季財報 ['2024-05']          
for p in range(0,1):  ##如果4月10號就可以找全部資料庫  變成44
    print(p)
...
'營業現金對負債比','營業現金對稅後純益比','return_rate','買入價','賣出價']
        ...
        #sellprice和return rate先隨便設一個數字帶入控充資料集會重抓價格
