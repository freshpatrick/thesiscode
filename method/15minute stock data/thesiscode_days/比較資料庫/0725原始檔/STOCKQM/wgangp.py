# -*- coding: utf-8 -*-
"""
Created on Sat Jun 22 23:26:37 2024
...
# Load data
path=r'D:/2021 4月開始的找回程式之旅/0409論文想做的題目/0506比較方法/新資料比較方法/WGAN課本範例/wgan_gp預測股價/data/'
...
yc_train = np.load(path+"yc_train.npy", allow_pickle=True)
yc_test = np.load(path+"yc_test.npy", allow_pickle=True)
...
yc_bigdata=np.load(r'D:/2021 4月開始的找回程式之旅/0409論文想做的題目/bigfinal_data/yc_data.npy')

##train0.8   test 0.2
indexs=np.random.permutation(len(x_bigdata)) #隨機排序
train_indexs=indexs[:int(len(x_bigdata)*0.8)]
...
X__test=x_bigdata_array[test_indexs]
#y部分
y_bigdata_array=np.array(y_bigdata).reshape(len(y_bigdata),1)
y_train=y_bigdata_array[train_indexs]
y_test=y_bigdata_array[test_indexs]
#yc部分
yc_train=yc_bigdata[train_indexs]
yc_test=yc_bigdata[test_indexs]
...
def Generator(input_dim, output_dim, feature_size) -> tf.keras.models.Model:# ->提示该函数 输入参数 和 返回值 的数据类型
    model = Sequential()
...
class GAN():
    def __init__(self, generator, discriminator):
        ...
        #將資料過程儲存起來
        checkpoint_dir = path+'/training_checkpoints1'
    ...
    def gradient_penalty(self, batch_size, real_output, fake_output):
        ...
        diff = fake_output - tf.cast(real_output, tf.float32) #tf.cast為數據轉換 ，fake和 real的差值
        #interpolated  插值
        ...
        gp = tf.reduce_mean((norm - 1.0) ** 2)  # gradient penalty
        return gp
    ...
    def train_step(self, data):
        ...
        for _ in range(1):
            with tf.GradientTape() as d_tape:
                ...
                d_loss = d_cost + gp * 10  #係數設為10

            d_grads = d_tape.gradient(d_loss, self.discriminator.trainable_variables)
            self.d_optimizer.apply_gradients(zip(d_grads, self.discriminator.trainable_variables))
        for _ in range(3):
            with tf.GradientTape() as g_tape:
                # Train the generator #訓練生成器
                # generate fake output  #餵給生成器真的圖片讓他產生假圖
                generated_data = self.generator(real_input, training=True)
                # reshape the data
                generated_data_reshape = tf.reshape(generated_data, [generated_data.shape[0], generated_data.shape[1], 1])
                fake_output = tf.concat([generated_data_reshape, tf.cast(yc, tf.float32)], axis=1)
    ...
    def train(self, X_train, y_train, yc, epochs):
        ...
        for epoch in range(epochs):
            ...
            # Save the model every 15 epochs  每執行一次就用
            if (epoch + 1) % 1 == 0:
                tf.keras.models.save_model(generator, path+'gen_GRU_model_%d.h5' % epoch)
...
#train_predict_index = np.load(path+"index_train.npy", allow_pickle=True)
#test_predict_index = np.load(path+"index_test.npy", allow_pickle=True)
